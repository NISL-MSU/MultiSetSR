...
def rosenbrock4D_pySR(X):
    ...
    return 109.24373217069 * (0.171526388718634 * (x0**2 - x1**2) ** 2 + 0.171526388718634 * (x2**2 - x3) ** 2 + 1) ** 3 - 88.20763
...
def A1_NAGAEq(X):
    x0, x1 = X[:, 0], X[:, 1]
    return 0.151096344284682*np.exp(1.5026*x0) - 0.499895632667704*np.sin(3.0047*x1 - 1.5715) - 0.000707859996299798
...
def S8_NAGAEq(X):
    x0, x1, x2 = X[:, 0], X[:, 1], X[:, 2]
    return 2.0*x0**2 - 4.0178*x0 - np.exp(0.9988*x1) * 1/(np. sin(6.5719 * np.exp(0.9655*x2) + 21.7291) + 1.4378) + 7.49
