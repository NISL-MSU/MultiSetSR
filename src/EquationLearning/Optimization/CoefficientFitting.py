...
class CoefficientFitting(Problem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for s in range(x.shape[0]):
            ...
            fs = set_args(self.skeleton, list(c[s, :]))
            ...
            if len(np.argwhere((np.isinf(ys)) | (np.isnan(ys)) | (np.abs(ys) > 10000))) > 0:
                error += 1000
            else:
                error += np.mean((self.y_est - ys)**2) * (2 - pearsonr(self.y_est, ys)[0])
...
class FitGA:
    ...
    def run(self):
        # Fit coefficients
        termination = RobustTermination(MultiObjectiveSpaceTermination(tol=1e-5), period=10)
