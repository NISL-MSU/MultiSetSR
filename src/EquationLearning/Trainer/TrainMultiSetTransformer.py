...
class TransformerTrainer:
    ...
    def __init__(self):
        ...
        # Read all equations
        self.sampledData_train_path = 'src/EquationLearning/Data/sampled_data/training'
        self.sampledData_val_path = 'src/EquationLearning/Data/sampled_data/validation'
    ...
    def fit(self):
        ...
        for epoch in trange(epochs):  # Epoch loop
            ...
            for b_ind in indexes[:0]:  # Block loop (each block contains 1000 inputs)
                ...
                XY_block = torch.zeros((len(block), block[0][0].shape[0], 2, block[0][0].shape[1])).to(self.device)
                skeletons_block = []
                for ib, b in enumerate(block):
                    Xs, Ys, tokenized, xpr = b
